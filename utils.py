...
import pandas as pd  # [필수] pandas가 import 되어 있어야 합니다.

# =================================================================
# [Monkey Patch] RecBole - Pandas 호환성 해결 코드 (여기부터 복사)
# 설명: RecBole이 DataFrame.shuffle()을 호출할 때 에러가 나지 않도록
#       Pandas DataFrame에 shuffle 기능을 강제로 주입합니다.
# =================================================================
if not hasattr(pd.DataFrame, 'shuffle'):
    def _shuffle_proxy(self):
        # shuffle() 대신 sample(frac=1)을 사용하여 데이터를 100% 랜덤하게 섞음
        return self.sample(frac=1).reset_index(drop=True)
    
    # DataFrame에 함수 등록
    pd.DataFrame.shuffle = _shuffle_proxy
    print(">>> [System] Pandas DataFrame에 'shuffle' 메서드를 성공적으로 주입했습니다.")
# =================================================================

# 2. Series.numpy 패치 (★지금 추가할 것★)
# 설명: RecBole이 Pandas Series를 Tensor로 착각하고 .numpy()를 호출할 때
#       에러 대신 .values를 반환하도록 가르칩니다.
if not hasattr(pd.Series, 'numpy'):
    def _numpy_proxy(self):
        return self.values
    pd.Series.numpy = _numpy_proxy
    print(">>> [System] Pandas Series에 'numpy' 메서드 주입 완료.")
...
# 설정에 따라 저장된 데이터셋을 로드하거나, 새로운 데이터셋 객체를 생성하여 데이터를 읽고 필터링
...
## 데이터셋을 학습/검증/테스트 세트로 분할하고, 이 분할된 데이터를 처리할 DataLoader와 Sampler를 생성
...
# 현재 모델(config["model"])의 종류와 단계(phase)에 따라 적절한 DataLoader 클래스를 반환
...
# 데이터 샘플링 방식을 결정하는 Sampler 객체 생성
